# MeritGrid: AI Development Guidelines (Gemini/Antigravity)

This document provides specialized instructions for Gemini/Antigravity AI agents to maintain MeritGrid's architectural integrity and premium quality.
...
- **Stateless Services**: All services MUST inherit from `BaseService`.
- **Caching**: Use Redis for data that is read-heavy and globally distributed.
- **Pagination**: Never return large arrays; always implement cursors or indexed offsets.

## 2. Security Protocols

### Authentication & Authorization
- **NextAuth.js**: Use `next-auth` for all authentication flows. Never roll custom auth.
- **RBAC Enforcement**: Check roles (`STUDENT`, `TALENT`, `ORG`, `ADMIN`) on every protected route.
...
- **SQL/NoSQL Injection**: Always use parameterized queries via Prisma/Mongoose. Never interpolate user input.

### Data Protection
- **Encryption at Rest**: Sensitive fields (SSN, bank details) MUST be encrypted using `Security.encrypt()`.
- **Encryption in Transit**: Enforce HTTPS everywhere. Use HSTS headers.
- **PII Masking**: Mask PII with `Security.maskPII()` before logging. Never log passwords, tokens, or secrets.
- **Data Minimization**: Collect only necessary data. Implement retention policies for old records.

### Error Handling
- **Sanitized Errors**: ALWAYS use `this.handleError(error, context)` in services. Never expose stack traces or DB errors to clients.
- **Error Codes**: Use standardized error codes from `lib/errors/codes.ts`. Return generic messages to users.

### API Security
- **Rate Limiting**: Apply per-user and per-IP limits using Redis. Return `429 Too Many Requests` on breach.
- **CORS**: Configure strict CORS policies. Only allow known origins.
...
- **Key Rotation**: Design systems to support credential rotation without downtime.
- **Audit Logging**: Log all access to sensitive endpoints with user ID, timestamp, and action.


## 3. UI/UX Standards (Premium Tier)
...
## 4. Response Practices

- **Conciseness**: Follow the `artifact_formatting_guidelines` strictly.
- **Direct Action**: Proactively run builds and lint checks (`npm run build`) before pushing changes.
- **Verification**: Document all proof of work in `walkthrough.md` with absolute file URIs.

## 5. Environment Mode (`NODE_ENV`)

All policies in this document target **production-level quality**. However, when `NODE_ENV=development`, the following relaxations apply to support iterative, phase-wise development:

| Policy Area | Production (`NODE_ENV=production`) | Development (`NODE_ENV=development`) |
|---|---|---|
| **Validation** | Strict Zod schema enforcement for all mutations. | Zod validation is encouraged, but can be bypassed for rapid prototyping. |
| **Security** | Full RBAC, rate limiting, and PII masking required. | RBAC and rate limiting can be disabled or mocked. PII masking is optional. |
| **Error Handling** | All errors MUST be sanitized via `this.handleError()`. | Verbose, unsanitized errors are acceptable for debugging. |
| **Data Fetching** | All data flows from DB. | Demo data fallback is the primary source until DB is connected. |
| **Caching** | Redis caching is mandatory for read-heavy paths. | Caching can be skipped entirely. |

> [!NOTE]
> When implementing a feature, always write the production-ready code path first. Then, add a conditional check for `process.env.NODE_ENV === 'development'` to enable the relaxed behavior. This ensures minimal refactoring when moving to production.

## 6. Code Quality & Collaboration

To ensure the codebase is maintainable, scalable, and allows multiple developers to work simultaneously:

- **Documentation**: All functions, classes, and complex logic blocks MUST have JSDoc/TSDoc comments explaining their purpose, parameters, and return values.
- **Modularity**: Break down features into small, self-contained modules. Each file should have a single responsibility. Avoid monolithic files exceeding 300 lines.
- **Clear Interfaces**: Define explicit `types/` or `interfaces/` for data contracts between modules. This reduces coupling and allows parallel development.
- **Folder Structure**: Organize code by feature or domain (e.g., `components/auth/`, `services/scholarships/`). Avoid dumping unrelated files into shared folders.
- **Readable Code**: Prioritize readability over cleverness. Use descriptive variable names and avoid deep nesting.
- **Verified Dependencies**: Only install **latest, stable, and verified** npm packages. Avoid outdated, deprecated, or unmaintained packages. Check npm download stats and last publish date before adding new dependencies.
...
- **Anti-Hallucination**: Never invent APIs, library functions, or configuration options. If unsure about a package's API, check the official documentation or existing codebase usage first. If still uncertain, ask for clarification rather than guessing. Admit uncertainty instead of fabricating.
